...
local moveDir = (pt.Position - rootPart.Position)
...
local dist = (Vector3.new(cPos.X, 0, cPos.Z) - Vector3.new(pt.Position.X, 0, pt.Position.Z)).Magnitude
if dist <= 2.0 or (os.clock() - startTime) > timeOut then
break
end
local dir = (pt.Position - cPos)
...
local dir = (pt.Position - rootPart.Position)
if dir.Magnitude > 0.1 then
humanoid:Move(dir.Unit, false)
end
rootPart.AssemblyLinearVelocity = Vector3.zero
