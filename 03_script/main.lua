...
if parentGui:FindFirstChild(_d({30,61,74,61,74,61,41,75,74,71,65,85,36,81,62,35,81,69},36)) then
parentGui.BananaMonkeyHubGui:Destroy()
end
local ScreenGui = Instance.new(_d({47,63,78,65,65,74,35,81,69},36))
ScreenGui.Name = _d({30,61,74,61,74,61,41,75,74,71,65,85,36,81,62,35,81,69},36)
...
local ToggleBtn = Instance.new(_d({48,65,84,80,30,81,80,80,75,74},36))
ToggleBtn.Name = _d({34,72,75,61,80,69,74,67,48,75,67,67,72,65},36)
ToggleBtn.Size = UDim2.new(0, 50, 0, 50)
ToggleBtn.Position = UDim2.new(0.03, 0, 0.25, 0)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
ToggleBtn.Text = _d({204,123,105,104},36)
...
local toggleCorner = Instance.new(_d({49,37,31,75,78,74,65,78},36))
toggleCorner.CornerRadius = UDim.new(0.5, 0)
toggleCorner.Parent = ToggleBtn
local toggleStroke = Instance.new(_d({49,37,47,80,78,75,71,65},36))
toggleStroke.Color = Color3.fromRGB(255, 255, 255)
toggleStroke.Thickness = 2
toggleStroke.Parent = ToggleBtn
local MainFrame = Instance.new(_d({34,78,61,73,65},36))
MainFrame.Name = _d({41,61,69,74,34,78,61,73,65},36)
...
local mainCorner = Instance.new(_d({49,37,31,75,78,74,65,78},36))
mainCorner.CornerRadius = UDim.new(0, 12)
mainCorner.Parent = MainFrame
local mainStroke = Instance.new(_d({49,37,47,80,78,75,71,65},36))
mainStroke.Color = Color3.fromRGB(255, 165, 0)
mainStroke.Thickness = 1.5
mainStroke.Parent = MainFrame
local Header = Instance.new(_d({48,65,84,80,40,61,62,65,72},36))
Header.Size = UDim2.new(1, -40, 0, 36)
Header.Position = UDim2.new(0, 12, 0, 4)
Header.BackgroundTransparency = 1
Header.Text = _d({30,61,74,61,74,61,252,41,75,74,71,65,85,252,36,81,62,252,204,123,105,104},36)
Header.TextColor3 = Color3.fromRGB(255, 255, 255)
Header.TextSize = 18
Header.Font = Enum.Font.SourceSansBold
Header.TextXAlignment = Enum.TextXAlignment.Left
Header.Parent = MainFrame
local Subtitle = Instance.new(_d({48,65,84,80,40,61,62,65,72},36))
Subtitle.Size = UDim2.new(1, -40, 0, 16)
Subtitle.Position = UDim2.new(0, 12, 0, 32)
Subtitle.BackgroundTransparency = 1
Subtitle.Text = _d({29,81,80,75,9,32,65,79,80,78,75,85,8,252,38,81,73,76,252,2,252,51,61,72,71,252,29,78,75,81,74,64},36)
...
local closeCorner = Instance.new(_d({49,37,31,75,78,74,65,78},36))
...
local Divider = Instance.new(_d({34,78,61,73,65},36))
Divider.Size = UDim2.new(1, -24, 0, 1)
Divider.Position = UDim2.new(0, 12, 0, 52)
Divider.BackgroundColor3 = Color3.fromRGB(45, 45, 60)
Divider.BorderSizePixel = 0
Divider.Parent = MainFrame
local AutoBtn = Instance.new(_d({48,65,84,80,30,81,80,80,75,74},36))
AutoBtn.Size = UDim2.new(1, -24, 0, 38)
AutoBtn.Position = UDim2.new(0, 12, 0, 60)
AutoBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
AutoBtn.Text = _d({29,81,80,75,9,32,65,79,80,78,75,85,252,35,78,75,81,74,64,22,252,43,34,34},36)
AutoBtn.TextColor3 = Color3.fromRGB(255, 90, 90)
AutoBtn.TextSize = 14
AutoBtn.Font = Enum.Font.SourceSansBold
AutoBtn.Parent = MainFrame
local autoCorner = Instance.new(_d({49,37,31,75,78,74,65,78},36))
autoCorner.CornerRadius = UDim.new(0, 8)
autoCorner.Parent = AutoBtn
local autoStroke = Instance.new(_d({49,37,47,80,78,75,71,65},36))
...
AutoBtn.Text = _d({29,81,80,75,9,32,65,79,80,78,75,85,252,35,78,75,81,74,64,22,252,43,42,252,190,118,125},36)
...
if character then
local root = character:FindFirstChild(_d({36,81,73,61,74,75,69,64,46,75,75,80,44,61,78,80},36))
if root then
local punchEvent = ReplicatedStorage:FindFirstChild(_d({32,65,79,80,78,81,63,80,69,75,74,59,44,81,74,63,68},36), true)
if punchEvent and punchEvent:IsA(_d({46,65,73,75,80,65,33,82,65,74,80},36)) then
...
local JumpBtn = Instance.new(_d({48,65,84,80,30,81,80,80,75,74},36))
JumpBtn.Size = UDim2.new(1, -24, 0, 32)
JumpBtn.Position = UDim2.new(0, 12, 0, 104)
JumpBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
JumpBtn.Text = _d({29,81,80,75,252,38,81,73,76,22,252,43,34,34},36)
JumpBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
JumpBtn.TextSize = 13
JumpBtn.Font = Enum.Font.SourceSansBold
JumpBtn.Parent = MainFrame
local jumpCorner = Instance.new(_d({49,37,31,75,78,74,65,78},36))
jumpCorner.CornerRadius = UDim.new(0, 8)
jumpCorner.Parent = JumpBtn
local jumpStroke = Instance.new(_d({49,37,47,80,78,75,71,65},36))
...
if autoJumping then
JumpBtn.Text = _d({29,81,80,75,252,38,81,73,76,22,252,43,42,252,204,123,130,116},36)
...
local character = LocalPlayer.Character
if character then
local humanoid = character:FindFirstChildOfClass(_d({36,81,73,61,74,75,69,64},36))
if humanoid then
pcall(function()
...
if autoMoving then
MoveBtn.Text = _d({29,81,80,75,9,41,75,82,65,252,4,51,61,72,71,252,29,78,75,81,74,64,5,22,252,43,42,252,204,123,118,146},36)
...
local SpeedLabel = Instance.new(_d({48,65,84,80,40,61,62,65,72},36))
SpeedLabel.Size = UDim2.new(1, -24, 0, 18)
SpeedLabel.Position = UDim2.new(0, 12, 0, 180)
SpeedLabel.BackgroundTransparency = 1
SpeedLabel.Text = _d({44,81,74,63,68,252,32,65,72,61,85,22,252,12,10,13,12,79,252,4,34,61,79,80,5},36)
SpeedLabel.TextColor3 = Color3.fromRGB(200, 200, 220)
SpeedLabel.TextSize = 12
SpeedLabel.Font = Enum.Font.SourceSans
SpeedLabel.TextXAlignment = Enum.TextXAlignment.Left
SpeedLabel.Parent = MainFrame
local speedContainer = Instance.new(_d({34,78,61,73,65},36))
speedContainer.Size = UDim2.new(1, -24, 0, 28)
speedContainer.Position = UDim2.new(0, 12, 0, 200)
speedContainer.BackgroundTransparency = 1
speedContainer.Parent = MainFrame
local speeds = {
{ label = _d({12,10,12,17,79},36), delay = 0.05 },
{ label = _d({12,10,13,12,79},36), delay = 0.10 },
{ label = _d({12,10,14,17,79},36), delay = 0.25 },
{ label = _d({12,10,17,12,79},36), delay = 0.50 }
}
...
SpeedLabel.Text = _d({44,81,74,63,68,252,32,65,72,61,85,22,252},36) .. string.format(_d({1,10,14,66,79},36), destroyDelay)
for _, child in ipairs(speedContainer:GetChildren()) do
if child:IsA(_d({48,65,84,80,30,81,80,80,75,74},36)) then
...
local DestroyBtn = Instance.new(_d({48,65,84,80,30,81,80,80,75,74},36))
...
DestroyBtn.Text = _d({49,74,72,75,61,64,252,36,81,62,252,47,63,78,69,76,80},36)
DestroyBtn.TextColor3 = Color3.fromRGB(255, 120, 120)
DestroyBtn.TextSize = 12
DestroyBtn.Font = Enum.Font.SourceSans
DestroyBtn.Parent = MainFrame
local destroyCorner = Instance.new(_d({49,37,31,75,78,74,65,78},36))
...
print(_d({55,30,61,74,61,74,61,252,41,75,74,71,65,85,252,36,81,62,57,252,40,75,61,64,65,64,252,79,81,63,63,65,79,79,66,81,72,72,85,253},36))
end)()