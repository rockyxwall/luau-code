...
if parentGui:FindFirstChild(_d({43,74,87,74,87,74,54,88,87,84,78,98,49,94,75,48,94,82},23)) then
parentGui.BananaMonkeyHubGui:Destroy()
end
local ScreenGui = Instance.new(_d({60,76,91,78,78,87,48,94,82},23))
ScreenGui.Name = _d({43,74,87,74,87,74,54,88,87,84,78,98,49,94,75,48,94,82},23)
...
local ToggleBtn = Instance.new(_d({61,78,97,93,43,94,93,93,88,87},23))
ToggleBtn.Name = _d({47,85,88,74,93,82,87,80,61,88,80,80,85,78},23)
ToggleBtn.Size = UDim2.new(0, 50, 0, 50)
ToggleBtn.Position = UDim2.new(0.03, 0, 0.25, 0)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
ToggleBtn.Text = _d({217,136,118,117},23)
...
local toggleCorner = Instance.new(_d({62,50,44,88,91,87,78,91},23))
toggleCorner.CornerRadius = UDim.new(0.5, 0)
toggleCorner.Parent = ToggleBtn
local toggleStroke = Instance.new(_d({62,50,60,93,91,88,84,78},23))
toggleStroke.Color = Color3.fromRGB(255, 255, 255)
toggleStroke.Thickness = 2
toggleStroke.Parent = ToggleBtn
local MainFrame = Instance.new(_d({47,91,74,86,78},23))
MainFrame.Name = _d({54,74,82,87,47,91,74,86,78},23)
...
local mainCorner = Instance.new(_d({62,50,44,88,91,87,78,91},23))
mainCorner.CornerRadius = UDim.new(0, 12)
mainCorner.Parent = MainFrame
local mainStroke = Instance.new(_d({62,50,60,93,91,88,84,78},23))
mainStroke.Color = Color3.fromRGB(255, 165, 0)
mainStroke.Thickness = 1.5
mainStroke.Parent = MainFrame
local Header = Instance.new(_d({61,78,97,93,53,74,75,78,85},23))
Header.Size = UDim2.new(1, -40, 0, 36)
Header.Position = UDim2.new(0, 12, 0, 4)
Header.BackgroundTransparency = 1
Header.Text = _d({43,74,87,74,87,74,9,54,88,87,84,78,98,9,49,94,75,9,217,136,118,117},23)
Header.TextColor3 = Color3.fromRGB(255, 255, 255)
Header.TextSize = 18
Header.Font = Enum.Font.SourceSansBold
Header.TextXAlignment = Enum.TextXAlignment.Left
Header.Parent = MainFrame
local Subtitle = Instance.new(_d({61,78,97,93,53,74,75,78,85},23))
Subtitle.Size = UDim2.new(1, -40, 0, 16)
Subtitle.Position = UDim2.new(0, 12, 0, 32)
Subtitle.BackgroundTransparency = 1
Subtitle.Text = _d({42,94,93,88,22,45,78,92,93,91,88,98,21,9,51,94,86,89,9,15,9,64,74,85,84,9,42,91,88,94,87,77},23)
...
local closeCorner = Instance.new(_d({62,50,44,88,91,87,78,91},23))
...
local Divider = Instance.new(_d({47,91,74,86,78},23))
Divider.Size = UDim2.new(1, -24, 0, 1)
Divider.Position = UDim2.new(0, 12, 0, 52)
Divider.BackgroundColor3 = Color3.fromRGB(45, 45, 60)
Divider.BorderSizePixel = 0
Divider.Parent = MainFrame
local AutoBtn = Instance.new(_d({61,78,97,93,43,94,93,93,88,87},23))
AutoBtn.Size = UDim2.new(1, -24, 0, 38)
AutoBtn.Position = UDim2.new(0, 12, 0, 60)
AutoBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
AutoBtn.Text = _d({42,94,93,88,22,45,78,92,93,91,88,98,9,48,91,88,94,87,77,35,9,56,47,47},23)
AutoBtn.TextColor3 = Color3.fromRGB(255, 90, 90)
AutoBtn.TextSize = 14
AutoBtn.Font = Enum.Font.SourceSansBold
AutoBtn.Parent = MainFrame
local autoCorner = Instance.new(_d({62,50,44,88,91,87,78,91},23))
autoCorner.CornerRadius = UDim.new(0, 8)
autoCorner.Parent = AutoBtn
local autoStroke = Instance.new(_d({62,50,60,93,91,88,84,78},23))
...
AutoBtn.Text = _d({42,94,93,88,22,45,78,92,93,91,88,98,9,48,91,88,94,87,77,35,9,56,55,9,203,131,138},23)
...
if character then
local root = character:FindFirstChild(_d({49,94,86,74,87,88,82,77,59,88,88,93,57,74,91,93},23))
if root then
local punchEvent = ReplicatedStorage:FindFirstChild(_d({45,78,92,93,91,94,76,93,82,88,87,72,57,94,87,76,81},23), true)
if punchEvent and punchEvent:IsA(_d({59,78,86,88,93,78,46,95,78,87,93},23)) then
...
local JumpBtn = Instance.new(_d({61,78,97,93,43,94,93,93,88,87},23))
JumpBtn.Size = UDim2.new(1, -24, 0, 32)
JumpBtn.Position = UDim2.new(0, 12, 0, 104)
JumpBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
JumpBtn.Text = _d({42,94,93,88,9,51,94,86,89,35,9,56,47,47},23)
JumpBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
JumpBtn.TextSize = 13
JumpBtn.Font = Enum.Font.SourceSansBold
JumpBtn.Parent = MainFrame
local jumpCorner = Instance.new(_d({62,50,44,88,91,87,78,91},23))
jumpCorner.CornerRadius = UDim.new(0, 8)
jumpCorner.Parent = JumpBtn
local jumpStroke = Instance.new(_d({62,50,60,93,91,88,84,78},23))
...
if autoJumping then
JumpBtn.Text = _d({42,94,93,88,9,51,94,86,89,35,9,56,55,9,217,136,143,129},23)
...
local character = LocalPlayer.Character
if character then
local humanoid = character:FindFirstChildOfClass(_d({49,94,86,74,87,88,82,77},23))
if humanoid then
pcall(function()
...
if autoMoving then
MoveBtn.Text = _d({42,94,93,88,22,54,88,95,78,9,17,64,74,85,84,9,42,91,88,94,87,77,18,35,9,56,55,9,217,136,131,159},23)
...
local SpeedLabel = Instance.new(_d({61,78,97,93,53,74,75,78,85},23))
SpeedLabel.Size = UDim2.new(1, -24, 0, 18)
SpeedLabel.Position = UDim2.new(0, 12, 0, 180)
SpeedLabel.BackgroundTransparency = 1
SpeedLabel.Text = _d({57,94,87,76,81,9,45,78,85,74,98,35,9,25,23,26,25,92,9,17,47,74,92,93,18},23)
SpeedLabel.TextColor3 = Color3.fromRGB(200, 200, 220)
SpeedLabel.TextSize = 12
SpeedLabel.Font = Enum.Font.SourceSans
SpeedLabel.TextXAlignment = Enum.TextXAlignment.Left
SpeedLabel.Parent = MainFrame
local speedContainer = Instance.new(_d({47,91,74,86,78},23))
speedContainer.Size = UDim2.new(1, -24, 0, 28)
speedContainer.Position = UDim2.new(0, 12, 0, 200)
speedContainer.BackgroundTransparency = 1
speedContainer.Parent = MainFrame
local speeds = {
{ label = _d({25,23,25,30,92},23), delay = 0.05 },
{ label = _d({25,23,26,25,92},23), delay = 0.10 },
{ label = _d({25,23,27,30,92},23), delay = 0.25 },
{ label = _d({25,23,30,25,92},23), delay = 0.50 }
}
...
SpeedLabel.Text = _d({57,94,87,76,81,9,45,78,85,74,98,35,9},23) .. string.format(_d({14,23,27,79,92},23), destroyDelay)
for _, child in ipairs(speedContainer:GetChildren()) do
if child:IsA(_d({61,78,97,93,43,94,93,93,88,87},23)) then
...
local DestroyBtn = Instance.new(_d({61,78,97,93,43,94,93,93,88,87},23))
...
DestroyBtn.Text = _d({62,87,85,88,74,77,9,49,94,75,9,60,76,91,82,89,93},23)
DestroyBtn.TextColor3 = Color3.fromRGB(255, 120, 120)
DestroyBtn.TextSize = 12
DestroyBtn.Font = Enum.Font.SourceSans
DestroyBtn.Parent = MainFrame
local destroyCorner = Instance.new(_d({62,50,44,88,91,87,78,91},23))
...
print(_d({68,43,74,87,74,87,74,9,54,88,87,84,78,98,9,49,94,75,70,9,53,88,74,77,78,77,9,92,94,76,76,78,92,92,79,94,85,85,98,10},23))
end)()