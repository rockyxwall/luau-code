...
if parentGui:FindFirstChild(_d({22,53,66,53,66,53,33,67,66,63,57,77,28,73,54,27,73,61},44)) then
parentGui.BananaMonkeyHubGui:Destroy()
end
local ScreenGui = Instance.new(_d({39,55,70,57,57,66,27,73,61},44))
ScreenGui.Name = _d({22,53,66,53,66,53,33,67,66,63,57,77,28,73,54,27,73,61},44)
...
local ToggleBtn = Instance.new(_d({40,57,76,72,22,73,72,72,67,66},44))
ToggleBtn.Name = _d({26,64,67,53,72,61,66,59,40,67,59,59,64,57},44)
ToggleBtn.Size = UDim2.new(0, 50, 0, 50)
ToggleBtn.Position = UDim2.new(0.03, 0, 0.25, 0)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
ToggleBtn.Text = _d({196,115,97,96},44)
...
local toggleCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
toggleCorner.CornerRadius = UDim.new(0.5, 0)
toggleCorner.Parent = ToggleBtn
local toggleStroke = Instance.new(_d({41,29,39,72,70,67,63,57},44))
toggleStroke.Color = Color3.fromRGB(255, 255, 255)
toggleStroke.Thickness = 2
toggleStroke.Parent = ToggleBtn
local MainFrame = Instance.new(_d({26,70,53,65,57},44))
MainFrame.Name = _d({33,53,61,66,26,70,53,65,57},44)
...
local mainCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
mainCorner.CornerRadius = UDim.new(0, 12)
mainCorner.Parent = MainFrame
local mainStroke = Instance.new(_d({41,29,39,72,70,67,63,57},44))
mainStroke.Color = Color3.fromRGB(255, 165, 0)
mainStroke.Thickness = 1.5
mainStroke.Parent = MainFrame
local Header = Instance.new(_d({40,57,76,72,32,53,54,57,64},44))
Header.Size = UDim2.new(1, -40, 0, 36)
Header.Position = UDim2.new(0, 12, 0, 4)
Header.BackgroundTransparency = 1
Header.Text = _d({22,53,66,53,66,53,244,33,67,66,63,57,77,244,28,73,54,244,196,115,97,96},44)
Header.TextColor3 = Color3.fromRGB(255, 255, 255)
Header.TextSize = 18
Header.Font = Enum.Font.SourceSansBold
Header.TextXAlignment = Enum.TextXAlignment.Left
Header.Parent = MainFrame
local Subtitle = Instance.new(_d({40,57,76,72,32,53,54,57,64},44))
Subtitle.Size = UDim2.new(1, -40, 0, 16)
Subtitle.Position = UDim2.new(0, 12, 0, 32)
Subtitle.BackgroundTransparency = 1
Subtitle.Text = _d({21,73,72,67,1,24,57,71,72,70,67,77,0,244,30,73,65,68,244,250,244,43,53,64,63,244,21,70,67,73,66,56},44)
...
local closeCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
...
local Divider = Instance.new(_d({26,70,53,65,57},44))
Divider.Size = UDim2.new(1, -24, 0, 1)
Divider.Position = UDim2.new(0, 12, 0, 52)
Divider.BackgroundColor3 = Color3.fromRGB(45, 45, 60)
Divider.BorderSizePixel = 0
Divider.Parent = MainFrame
local AutoBtn = Instance.new(_d({40,57,76,72,22,73,72,72,67,66},44))
AutoBtn.Size = UDim2.new(1, -24, 0, 38)
AutoBtn.Position = UDim2.new(0, 12, 0, 60)
AutoBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
AutoBtn.Text = _d({21,73,72,67,1,24,57,71,72,70,67,77,244,27,70,67,73,66,56,14,244,35,26,26},44)
AutoBtn.TextColor3 = Color3.fromRGB(255, 90, 90)
AutoBtn.TextSize = 14
AutoBtn.Font = Enum.Font.SourceSansBold
AutoBtn.Parent = MainFrame
local autoCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
autoCorner.CornerRadius = UDim.new(0, 8)
autoCorner.Parent = AutoBtn
local autoStroke = Instance.new(_d({41,29,39,72,70,67,63,57},44))
...
AutoBtn.Text = _d({21,73,72,67,1,24,57,71,72,70,67,77,244,27,70,67,73,66,56,14,244,35,34,244,182,110,117},44)
...
if character then
local root = character:FindFirstChild(_d({28,73,65,53,66,67,61,56,38,67,67,72,36,53,70,72},44))
if root then
local punchEvent = ReplicatedStorage:FindFirstChild(_d({24,57,71,72,70,73,55,72,61,67,66,51,36,73,66,55,60},44), true)
if punchEvent and punchEvent:IsA(_d({38,57,65,67,72,57,25,74,57,66,72},44)) then
...
local JumpBtn = Instance.new(_d({40,57,76,72,22,73,72,72,67,66},44))
JumpBtn.Size = UDim2.new(1, -24, 0, 32)
JumpBtn.Position = UDim2.new(0, 12, 0, 104)
JumpBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
JumpBtn.Text = _d({21,73,72,67,244,30,73,65,68,14,244,35,26,26},44)
JumpBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
JumpBtn.TextSize = 13
JumpBtn.Font = Enum.Font.SourceSansBold
JumpBtn.Parent = MainFrame
local jumpCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
jumpCorner.CornerRadius = UDim.new(0, 8)
jumpCorner.Parent = JumpBtn
local jumpStroke = Instance.new(_d({41,29,39,72,70,67,63,57},44))
...
if autoJumping then
JumpBtn.Text = _d({21,73,72,67,244,30,73,65,68,14,244,35,34,244,196,115,122,108},44)
...
local character = LocalPlayer.Character
if character then
local humanoid = character:FindFirstChildOfClass(_d({28,73,65,53,66,67,61,56},44))
if humanoid then
pcall(function()
...
local MoveBtn = Instance.new(_d({40,57,76,72,22,73,72,72,67,66},44))
MoveBtn.Size = UDim2.new(1, -24, 0, 32)
MoveBtn.Position = UDim2.new(0, 12, 0, 142)
MoveBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
MoveBtn.Text = _d({21,73,72,67,1,33,67,74,57,244,252,43,53,64,63,244,21,70,67,73,66,56,253,14,244,35,26,26},44)
MoveBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
MoveBtn.TextSize = 13
MoveBtn.Font = Enum.Font.SourceSansBold
MoveBtn.Parent = MainFrame
local moveCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
moveCorner.CornerRadius = UDim.new(0, 8)
moveCorner.Parent = MoveBtn
local moveStroke = Instance.new(_d({41,29,39,72,70,67,63,57},44))
...
if autoMoving then
MoveBtn.Text = _d({21,73,72,67,1,33,67,74,57,244,252,43,53,64,63,244,21,70,67,73,66,56,253,14,244,35,34,244,196,115,110,138},44)
...
local SpeedLabel = Instance.new(_d({40,57,76,72,32,53,54,57,64},44))
SpeedLabel.Size = UDim2.new(1, -24, 0, 18)
SpeedLabel.Position = UDim2.new(0, 12, 0, 180)
SpeedLabel.BackgroundTransparency = 1
SpeedLabel.Text = _d({36,73,66,55,60,244,24,57,64,53,77,14,244,4,2,5,4,71,244,252,26,53,71,72,253},44)
SpeedLabel.TextColor3 = Color3.fromRGB(200, 200, 220)
SpeedLabel.TextSize = 12
SpeedLabel.Font = Enum.Font.SourceSans
SpeedLabel.TextXAlignment = Enum.TextXAlignment.Left
SpeedLabel.Parent = MainFrame
local speedContainer = Instance.new(_d({26,70,53,65,57},44))
speedContainer.Size = UDim2.new(1, -24, 0, 28)
speedContainer.Position = UDim2.new(0, 12, 0, 200)
speedContainer.BackgroundTransparency = 1
speedContainer.Parent = MainFrame
local speeds = {
{ label = _d({4,2,4,9,71},44), delay = 0.05 },
{ label = _d({4,2,5,4,71},44), delay = 0.10 },
{ label = _d({4,2,6,9,71},44), delay = 0.25 },
{ label = _d({4,2,9,4,71},44), delay = 0.50 }
}
...
SpeedLabel.Text = _d({36,73,66,55,60,244,24,57,64,53,77,14,244},44) .. string.format(_d({249,2,6,58,71},44), destroyDelay)
for _, child in ipairs(speedContainer:GetChildren()) do
if child:IsA(_d({40,57,76,72,22,73,72,72,67,66},44)) then
...
local DestroyBtn = Instance.new(_d({40,57,76,72,22,73,72,72,67,66},44))
...
DestroyBtn.Text = _d({41,66,64,67,53,56,244,28,73,54,244,39,55,70,61,68,72},44)
DestroyBtn.TextColor3 = Color3.fromRGB(255, 120, 120)
DestroyBtn.TextSize = 12
DestroyBtn.Font = Enum.Font.SourceSans
DestroyBtn.Parent = MainFrame
local destroyCorner = Instance.new(_d({41,29,23,67,70,66,57,70},44))
...
print(_d({47,22,53,66,53,66,53,244,33,67,66,63,57,77,244,28,73,54,49,244,32,67,53,56,57,56,244,71,73,55,55,57,71,71,58,73,64,64,77,245},44))
end)()