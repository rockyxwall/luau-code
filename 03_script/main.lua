...
if parentGui:FindFirstChild(_d({49,80,93,80,93,80,60,94,93,90,84,104,55,100,81,54,100,88},17)) then
parentGui.BananaMonkeyHubGui:Destroy()
end
local ScreenGui = Instance.new(_d({66,82,97,84,84,93,54,100,88},17))
ScreenGui.Name = _d({49,80,93,80,93,80,60,94,93,90,84,104,55,100,81,54,100,88},17)
...
local ToggleBtn = Instance.new(_d({67,84,103,99,49,100,99,99,94,93},17))
ToggleBtn.Name = _d({53,91,94,80,99,88,93,86,67,94,86,86,91,84},17)
ToggleBtn.Size = UDim2.new(0, 50, 0, 50)
ToggleBtn.Position = UDim2.new(0.03, 0, 0.25, 0)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
ToggleBtn.Text = _d({223,142,124,123},17)
...
local toggleCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
toggleCorner.CornerRadius = UDim.new(0.5, 0)
toggleCorner.Parent = ToggleBtn
local toggleStroke = Instance.new(_d({68,56,66,99,97,94,90,84},17))
toggleStroke.Color = Color3.fromRGB(255, 255, 255)
toggleStroke.Thickness = 2
toggleStroke.Parent = ToggleBtn
local MainFrame = Instance.new(_d({53,97,80,92,84},17))
MainFrame.Name = _d({60,80,88,93,53,97,80,92,84},17)
...
local mainCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
mainCorner.CornerRadius = UDim.new(0, 12)
mainCorner.Parent = MainFrame
local mainStroke = Instance.new(_d({68,56,66,99,97,94,90,84},17))
mainStroke.Color = Color3.fromRGB(255, 165, 0)
mainStroke.Thickness = 1.5
mainStroke.Parent = MainFrame
local Header = Instance.new(_d({67,84,103,99,59,80,81,84,91},17))
Header.Size = UDim2.new(1, -40, 0, 36)
Header.Position = UDim2.new(0, 12, 0, 4)
Header.BackgroundTransparency = 1
Header.Text = _d({49,80,93,80,93,80,15,60,94,93,90,84,104,15,55,100,81,15,223,142,124,123},17)
Header.TextColor3 = Color3.fromRGB(255, 255, 255)
Header.TextSize = 18
Header.Font = Enum.Font.SourceSansBold
Header.TextXAlignment = Enum.TextXAlignment.Left
Header.Parent = MainFrame
local Subtitle = Instance.new(_d({67,84,103,99,59,80,81,84,91},17))
Subtitle.Size = UDim2.new(1, -40, 0, 16)
Subtitle.Position = UDim2.new(0, 12, 0, 32)
Subtitle.BackgroundTransparency = 1
Subtitle.Text = _d({48,100,99,94,28,51,84,98,99,97,94,104,27,15,57,100,92,95,15,21,15,70,80,91,90,15,48,97,94,100,93,83},17)
...
local closeCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
...
local Divider = Instance.new(_d({53,97,80,92,84},17))
Divider.Size = UDim2.new(1, -24, 0, 1)
Divider.Position = UDim2.new(0, 12, 0, 52)
Divider.BackgroundColor3 = Color3.fromRGB(45, 45, 60)
Divider.BorderSizePixel = 0
Divider.Parent = MainFrame
local AutoBtn = Instance.new(_d({67,84,103,99,49,100,99,99,94,93},17))
AutoBtn.Size = UDim2.new(1, -24, 0, 38)
AutoBtn.Position = UDim2.new(0, 12, 0, 60)
AutoBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
AutoBtn.Text = _d({48,100,99,94,28,51,84,98,99,97,94,104,15,54,97,94,100,93,83,41,15,62,53,53},17)
AutoBtn.TextColor3 = Color3.fromRGB(255, 90, 90)
AutoBtn.TextSize = 14
AutoBtn.Font = Enum.Font.SourceSansBold
AutoBtn.Parent = MainFrame
local autoCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
autoCorner.CornerRadius = UDim.new(0, 8)
autoCorner.Parent = AutoBtn
local autoStroke = Instance.new(_d({68,56,66,99,97,94,90,84},17))
...
AutoBtn.Text = _d({48,100,99,94,28,51,84,98,99,97,94,104,15,54,97,94,100,93,83,41,15,62,61,15,209,137,144},17)
...
if character then
local root = character:FindFirstChild(_d({55,100,92,80,93,94,88,83,65,94,94,99,63,80,97,99},17))
if root then
local punchEvent = ReplicatedStorage:FindFirstChild(_d({51,84,98,99,97,100,82,99,88,94,93,78,63,100,93,82,87},17), true)
if punchEvent and punchEvent:IsA(_d({65,84,92,94,99,84,52,101,84,93,99},17)) then
...
local JumpBtn = Instance.new(_d({67,84,103,99,49,100,99,99,94,93},17))
JumpBtn.Size = UDim2.new(1, -24, 0, 32)
JumpBtn.Position = UDim2.new(0, 12, 0, 104)
JumpBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
JumpBtn.Text = _d({48,100,99,94,15,57,100,92,95,41,15,62,53,53},17)
JumpBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
JumpBtn.TextSize = 13
JumpBtn.Font = Enum.Font.SourceSansBold
JumpBtn.Parent = MainFrame
local jumpCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
jumpCorner.CornerRadius = UDim.new(0, 8)
jumpCorner.Parent = JumpBtn
local jumpStroke = Instance.new(_d({68,56,66,99,97,94,90,84},17))
...
if autoJumping then
JumpBtn.Text = _d({48,100,99,94,15,57,100,92,95,41,15,62,61,15,223,142,149,135},17)
...
local character = LocalPlayer.Character
if character then
local humanoid = character:FindFirstChildOfClass(_d({55,100,92,80,93,94,88,83},17))
if humanoid then
pcall(function()
...
local MoveBtn = Instance.new(_d({67,84,103,99,49,100,99,99,94,93},17))
MoveBtn.Size = UDim2.new(1, -24, 0, 32)
MoveBtn.Position = UDim2.new(0, 12, 0, 142)
MoveBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
MoveBtn.Text = _d({48,100,99,94,28,60,94,101,84,15,23,70,80,91,90,15,48,97,94,100,93,83,24,41,15,62,53,53},17)
MoveBtn.TextColor3 = Color3.fromRGB(200, 200, 200)
MoveBtn.TextSize = 13
MoveBtn.Font = Enum.Font.SourceSansBold
MoveBtn.Parent = MainFrame
local moveCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
moveCorner.CornerRadius = UDim.new(0, 8)
moveCorner.Parent = MoveBtn
local moveStroke = Instance.new(_d({68,56,66,99,97,94,90,84},17))
...
if autoMoving then
MoveBtn.Text = _d({48,100,99,94,28,60,94,101,84,15,23,70,80,91,90,15,48,97,94,100,93,83,24,41,15,62,61,15,223,142,137,165},17)
...
local SpeedLabel = Instance.new(_d({67,84,103,99,59,80,81,84,91},17))
SpeedLabel.Size = UDim2.new(1, -24, 0, 18)
SpeedLabel.Position = UDim2.new(0, 12, 0, 180)
SpeedLabel.BackgroundTransparency = 1
SpeedLabel.Text = _d({63,100,93,82,87,15,51,84,91,80,104,41,15,31,29,32,31,98,15,23,53,80,98,99,24},17)
SpeedLabel.TextColor3 = Color3.fromRGB(200, 200, 220)
SpeedLabel.TextSize = 12
SpeedLabel.Font = Enum.Font.SourceSans
SpeedLabel.TextXAlignment = Enum.TextXAlignment.Left
SpeedLabel.Parent = MainFrame
local speedContainer = Instance.new(_d({53,97,80,92,84},17))
speedContainer.Size = UDim2.new(1, -24, 0, 28)
speedContainer.Position = UDim2.new(0, 12, 0, 200)
speedContainer.BackgroundTransparency = 1
speedContainer.Parent = MainFrame
local speeds = {
{ label = _d({31,29,31,36,98},17), delay = 0.05 },
{ label = _d({31,29,32,31,98},17), delay = 0.10 },
{ label = _d({31,29,33,36,98},17), delay = 0.25 },
{ label = _d({31,29,36,31,98},17), delay = 0.50 }
}
...
SpeedLabel.Text = _d({63,100,93,82,87,15,51,84,91,80,104,41,15},17) .. string.format(_d({20,29,33,85,98},17), destroyDelay)
for _, child in ipairs(speedContainer:GetChildren()) do
if child:IsA(_d({67,84,103,99,49,100,99,99,94,93},17)) then
...
local DestroyBtn = Instance.new(_d({67,84,103,99,49,100,99,99,94,93},17))
...
DestroyBtn.Text = _d({68,93,91,94,80,83,15,55,100,81,15,66,82,97,88,95,99},17)
DestroyBtn.TextColor3 = Color3.fromRGB(255, 120, 120)
DestroyBtn.TextSize = 12
DestroyBtn.Font = Enum.Font.SourceSans
DestroyBtn.Parent = MainFrame
local destroyCorner = Instance.new(_d({68,56,50,94,97,93,84,97},17))
...
print(_d({74,49,80,93,80,93,80,15,60,94,93,90,84,104,15,55,100,81,76,15,59,94,80,83,84,83,15,98,100,82,82,84,98,98,85,100,91,91,104,16},17))
end)()