...
local hotbarRemote = ReplicatedStorage.Events:FindFirstChild(_d({21,60,65,47,46,63},51))
...
if filtergc then
pcall(function()
local cache = filtergc(_d({65,46,47,57,50},51), { Keys = {_d({28,59,50},51), _d({33,68,60},51), _d({33,53,63,50,50},51)} }, true)
if cache and type(cache) == _d({65,46,47,57,50},51) then
...
if type(v) == _d({65,46,47,57,50},51) then
if rawget(v, _d({28,59,50},51)) ~= nil and rawget(v, _d({33,68,60},51)) ~= nil and rawget(v, _d({33,53,63,50,50},51)) ~= nil then
for slot, toolName in pairs(mapping) do
...
local slotsOrder = {_d({28,59,50},51), _d({33,68,60},51), _d({33,53,63,50,50},51), _d({19,60,66,63},51), _d({19,54,67,50},51), _d({32,54,69},51), _d({32,50,67,50,59},51), _d({18,54,52,53,65},51), _d({27,54,59,50},51), _d({39,50,63,60},51)}
for _, slot in ipairs(slotsOrder) do
if mapping[slot] == _d({27,60,59,50},51) then
currentSlot = slot
break
end
end
if not currentSlot then
currentSlot = _d({27,54,59,50},51)
end
mapping[currentSlot] = _d({31,54,51,57,50},51)
print(_d({40,20,50,61,60,237,20,63,54,59,49,50,63,42,237,15,54,59,49,54,59,52,237,31,54,51,57,50,237,65,60,237,53,60,65,47,46,63,237,64,57,60,65,7,237},51) .. tostring(currentSlot))
syncClientHotbar(mapping)
else
print(_d({40,20,50,61,60,237,20,63,54,59,49,50,63,42,237,31,54,51,57,50,237,54,64,237,46,57,63,50,46,49,70,237,58,46,61,61,50,49,237,65,60,237,53,60,65,47,46,63,237,64,57,60,65,7,237},51) .. tostring(currentSlot))
end
...
rifleTool = LocalPlayer.Backpack:FindFirstChild(_d({31,54,51,57,50},51)) or (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild(_d({31,54,51,57,50},51)))
if rifleTool then
break
end
end
if not rifleTool then
warn(_d({40,20,50,61,60,237,20,63,54,59,49,50,63,42,237,31,54,51,57,50,237,68,46,64,237,47,60,66,59,49,237,65,60,237,53,60,65,47,46,63,237,47,66,65,237,49,54,49,237,59,60,65,237,46,61,61,50,46,63,237,54,59,237,15,46,48,56,61,46,48,56,252,16,53,46,63,46,48,65,50,63,237,68,54,65,53,54,59,237,254,253,237,64,50,48,60,59,49,64,251},51))
cleanup(_d({31,54,51,57,50,237,63,50,61,57,54,48,46,65,54,60,59,237,65,54,58,50,60,66,65},51))
return
end
local finalRifle = LocalPlayer.Backpack:FindFirstChild(_d({31,54,51,57,50},51))
local hum = getHumanoid()
if finalRifle and hum then
hum:EquipTool(finalRifle)
print(_d({40,20,50,61,60,237,20,63,54,59,49,50,63,42,237,31,54,51,57,50,237,64,66,48,48,50,64,64,51,66,57,57,70,237,50,62,66,54,61,61,50,49,238},51))
end
