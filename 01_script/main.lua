...
debug(_d({30,58,47,71,51,64,238,50,55,51,50,239,238,33,66,61,62,62,55,60,53,238,48,61,66,252},50))
disableBot()
return
end
...
debug(_d({30,58,47,71,51,64,238,55,65,238,66,61,61,238,52,47,64,238,52,64,61,59,238,66,47,64,53,51,66,238,246,12,0,254,254,254,238,65,66,67,50,65,247,252,238,26,55,57,51,58,71,238,64,51,65,62,47,69,60,51,50,238,47,66,238,58,61,48,48,71,252,238,33,66,61,62,62,55,60,53,238,48,61,66,252},50))
disableBot()
return
end
...
debug(_d({34,61,61,57,238,50,47,59,47,53,51,238,69,54,55,58,51,238,69,47,58,57,55,60,53,238,66,61,238,62,61,55,60,66,239,238,33,66,61,62,62,55,60,53,238,69,47,58,57,238,66,61,238,51,60,53,47,53,51,252},50))
break
end
...
debug(_d({33,66,67,49,57,238,50,67,64,55,60,53,238,69,47,58,57,250,238,56,67,59,62,55,60,53,239},50))
stuckTicks += 1
VIM:SendKeyEvent(true, Enum.KeyCode.Space, false, game)
task.wait(0.05)
VIM:SendKeyEvent(false, Enum.KeyCode.Space, false, game)
if stuckTicks > 1 then
debug(_d({33,66,55,58,58,238,65,66,67,49,57,250,238,66,64,55,53,53,51,64,55,60,53,238,21,51,62,62,61,239},50))
task.wait(0.05)
VIM:SendKeyEvent(true, Enum.KeyCode.Space, false, game)
task.wait(0.05)
VIM:SendKeyEvent(false, Enum.KeyCode.Space, false, game)
...
task.wait(5)
debug(_d({37,47,55,66,55,60,53,238,52,61,64},50), targetHP * 100, _d({243,238,22,30,238,48,51,52,61,64,51,238,59,61,68,55,60,53,238,66,61,238,60,51,70,66,238,65,66,47,53,51},50))
local hum = getHumanoid()
if hum then
...
debug(_d({27,61,68,55,60,53,238,66,61,238,26,51,61},50))
equipSwordOrMelee()
...
debug(_d({26,51,61,238,65,66,47,64,66,51,50,238,48,58,61,49,57,251,48,64,51,47,57,51,64,238,59,55,50,251,48,58,61,49,57,239,238,19,68,47,50,55,60,53,252,252,252},50))
break
end
task.wait(0.1)
holdTime += 0.1
...
local hpTarget = (stage == _d({33,66,47,53,51,1,16},50)) and 0.40 or 0.95
clearStage(stage, hpTarget)
end
if not enabled then return end
debug(_d({27,61,68,55,60,53,238,66,61,238,47,64,64,61,69,238,52,58,71,251,50,61,69,60,238,47,64,51,47,238,246,17,67,62,55,50,238,32,47,55,60,247},50))
walkToPoint(COORDS.ArrowFlyDown, 30, true)
debug(_d({18,61,50,53,55,60,53,238,47,64,64,61,69,238,64,47,55,60,238,55,60,238,47,238,65,63,67,47,64,51},50))
local elapsed = 0
...
if not Safeguard.RequirePlace(11424731604, _d({17,67,62,55,50,238,18,67,60,53,51,61,60},50)) then
return
end
enableBot()
end
...
CupidDungeon,
_d({17,67,62,55,50,238,18,67,60,53,51,61,60},50),
CupidDungeon.Start,
CupidDungeon.Stop,
...
return char, char:FindFirstChildWhichIsA(_d({22,67,59,47,60,61,55,50},50)), char:FindFirstChild(_d({22,67,59,47,60,61,55,50,32,61,61,66,30,47,64,66},50))
end
local function getOrCreateForce(root)
...
local hitCave = false
local cave = Workspace.Islands:FindFirstChild(_d({20,55,65,54,59,47,60,238,17,47,68,51},50))
...
Core.SetupStandalone(
EasyTravel,
_d({19,47,65,71,238,34,64,47,68,51,58},50),
EasyTravel.Start,
EasyTravel.Stop,
...
local playerGui = LocalPlayer:WaitForChild(_d({30,58,47,71,51,64,21,67,55},50), 10)
if not playerGui then return end
local oldUI = playerGui:FindFirstChild(_d({21,30,29,26,47,67,60,49,54,51,64,35,23},50))
if oldUI then oldUI:Destroy() end
local screenGui = Instance.new(_d({33,49,64,51,51,60,21,67,55},50))
screenGui.Name = _d({21,30,29,26,47,67,60,49,54,51,64,35,23},50)
screenGui.ResetOnSpawn = false
screenGui.Parent = playerGui
...
local corner = Instance.new(_d({35,23,17,61,64,60,51,64},50))
...
title.TextXAlignment = Enum.TextXAlignment.Left
title.Text = _d({190,109,90,90,238,21,30,29,238,22,67,48,238,26,47,67,60,49,54,51,64},50)
title.Parent = main
...
local status = Instance.new(_d({34,51,70,66,26,47,48,51,58},50))
...
status.Text = _d({17,54,61,61,65,51,238,47,238,48,61,66,238,61,64,238,67,66,55,58,55,66,71,238,66,61,238,64,67,60,8},50)
status.Parent = main
local buttonCount = 0
...
btn.Text = _d({238,238},50) .. text
btn.TextXAlignment = Enum.TextXAlignment.Left
btn.Parent = main
local btnCorner = Instance.new(_d({35,23,17,61,64,60,51,64},50))
btnCorner.CornerRadius = UDim.new(0, 6)
btnCorner.Parent = btn
local btnStroke = Instance.new(_d({35,23,33,66,64,61,57,51},50))
btnStroke.Color = Color3.fromRGB(48, 52, 68)
btnStroke.Thickness = 1
btnStroke.Parent = btn
local descLabel = Instance.new(_d({34,51,70,66,26,47,48,51,58},50))
...
CreateLaunchButton(_d({17,67,62,55,50,238,18,67,60,53,51,61,60,238,20,47,64,59},50), _d({15,67,66,61,59,47,66,51,238,49,67,62,55,50,238,50,67,60,53,51,61,60,65,238,244,238,48,61,65,65,238,49,71,49,58,51,65},50), loadCupidDungeon)
CreateLaunchButton(_d({22,61,64,61,238,16,61,65,65,238,20,47,64,59,238,246,33,55,58,51,60,66,238,15,55,59,247},50), _d({15,67,66,61,52,47,64,59,238,61,68,51,64,69,61,64,58,50,238,48,61,65,65,51,65,238,67,65,55,60,53,238,22,61,64,61,238,52,64,67,55,66,65},50), loadHoroBossFarm)
CreateLaunchButton(_d({26,51,68,51,58,238,244,238,27,61,48,238,21,64,55,60,50,51,64},50), _d({15,67,66,61,251,58,51,68,51,58,238,47,60,50,238,52,47,64,59,238,58,61,49,47,58,238,28,30,17,238,59,61,48,65},50), loadLevelGrinder)
CreateLaunchButton(_d({19,47,65,71,238,34,64,47,68,51,58,238,246,30,238,34,61,53,53,58,51,247},50), _d({37,15,33,18,238,20,58,55,53,54,66,238,69,55,66,54,238,53,64,61,67,60,50,238,52,61,58,58,61,69,238,244,238,69,47,58,58,238,49,58,55,59,48,55,60,53},50), loadNavigationLab)
CreateLaunchButton(_d({30,54,71,65,55,49,65,238,29,68,51,64,69,61,64,58,50,238,34,51,65,66,51,64},50), _d({34,51,65,66,238,49,61,59,48,47,66,238,54,61,68,51,64,250,238,53,51,62,62,61,238,244,238,50,61,50,53,51,238,54,51,55,53,54,66,65},50), loadOverworldTester)
end
task.spawn(CreateLauncherUI)
print(_d({41,21,30,29,238,22,67,48,43,238,26,47,67,60,49,54,51,64,238,35,23,238,55,60,55,66,55,47,58,55,72,51,50,252},50))
end)()